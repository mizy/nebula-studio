...
const ngqlMap = ngqlDoc.reduce((acc, item) => {
  item.url = item.url.replace('https://docs.nebula-graph.io/3.5.0/3.ngql-guide/', '')
  acc[item.url] = item
  return acc
})
const categoryDict = "["+ngqlDoc.map(item => `"${item.url.replace('https://docs.nebula-graph.io/3.5.0/3.ngql-guide/', '')}"`).join(",")+"]"
...
function Chat() {
  ...
  const onSend = debounce(async () => {
    ...
    const sendMessages = [{
      role: 'system',
      content:"You are a helpful NebulaGraph database assistant to help user write the ngql. you need use markdown to reply short and clear."
    },
      // slice 100 char 
  ...
  async function GetDocPrompt(text) {
    ...
    if (res.code === 0) {
      ...
      if (ngqlMap[url]) {
        ...
        if(doc){
          prompt =  `learn the below nGQL, and use it to help user write the ngql,the user space schema is "{schema}", the user space vid type is "${schema.spaceVidType}" the doc is: \n"${doc.replaceAll("  ","").replaceAll("\n","").slice(0,1000)}"\n the question is "{query_str}",you need think more and more.and add \`\`\` as markdown code block to write the gql.`
        }
      }
    }
    ...
    const tagsSchema = tagList.map(item => {
      return `${item.name}[${item.fields.map(p => p.Field+`(${p.Type})`).join(',')}]`
    }).join('\n')
    const edgeTypesSchema = edgeList.map(item => {
      return `${item.name}[${item.fields.map(p => p.Field+`(${p.Type})`).join(',')}]`
    }).join('\n')

    prompt = prompt.replace('{schema}', "tags:\n:"+tagsSchema+'\nedges:\n'+edgeTypesSchema)
    return prompt
  }
    ...
    return gqls.map((item, index) => {
      if (index % 2 === 0) {
        ...
        return <div className={styles.codeWrapper}>
              <span onClick={()=>{runNgqlInConsole(item)}}>Run</span>
            <CodeMirror height={item.split('\n').length*30+''} value={item} options={{
                type:"nebula"
            }}/>
          </div>
      }
  ...
  return (
    <div className={styles.chat}> 
        ...
        <div className={styles.chatContentInner}>
          {
            messages.map((item, index) => {
              return <div key={index} className={styles.chatMessage+" "+styles[(item.role=="user"?"fromUser":"fromBot")]}>
      ...
      <div className={styles.chatInput}>
        ...
        <Button type="primary" size="small" onClick={onSend}>Send</Button>
      </div>
    </div>
  )
}
